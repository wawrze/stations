...
class StationDaoTestSuite : BaseDaoTestSuite() {
    ...
    fun shouldFetchStationByKeywordsStartingWith() {
        ...
        TestCase.assertEquals(2L, result[0].stationId)
        TestCase.assertEquals("station2", result[0].name)
        TestCase.assertEquals(1L, result[1].stationId)
        TestCase.assertEquals("station1", result[1].name)
    }
    ...
    fun shouldFetchStationsByKeywordsEndingWith() {
        ...
        // then
        TestCase.assertEquals(2, result.size)
        TestCase.assertEquals(3L, result[0].stationId)
        TestCase.assertEquals("station3", result[0].name)
        TestCase.assertEquals(1L, result[1].stationId)
        TestCase.assertEquals("station1", result[1].name)
    }
    ...
    @Test
    fun shouldFetchMatchingKeywordsContainingInMiddle() {
        // given
        val stations = listOf(
            Station(
                1L,
        ...
        objectUnderTest.updateStations(stations).blockingGet()
        objectUnderTest.updateKeywords(keywords).blockingGet()
        val result = objectUnderTest.getMatchingStations("es").blockingGet()
        // then
        TestCase.assertEquals(1, result.size)
        TestCase.assertEquals(4L, result[0].stationId)
        TestCase.assertEquals("station4", result[0].name)
